...
do isub=1,avg_blk%nsub
   do il0=1,geom%nl0
      ...
      if (nam%var_filter.and.(.not.nam%gau_approx)) then
         ...
         avg_blk%m4(il0,isub) = sum(mom_blk%m22(:,1,jl0r,il0,isub),mask=mpl%msv%isnotr(mom_blk%m22(:,1,jl0r,il0,isub)))
      end if
   end do
end do
...
do il0=1,geom%nl0
   do jl0r=1,bpar%nl0r(ib)
      ...
      do jc3=1,bpar%nc3(ib)
         ...
         ! Number of valid points
         nc1a = count(mpl%msv%isnotr(list_m11lrm11sub))
         avg_blk%nc1a(jc3,jl0r,il0) = real(nc1a,kind_real)
         ...
         if (nc1a>0) then
            do isub=1,avg_blk%nsub
               do jsub=1,avg_blk%nsub
                  avg_blk%m11lrm11sub(jc3,jl0r,il0,jsub,isub) = sum(list_m11lrm11sub,mask=mpl%msv%isnotr(list_m11lrm11sub))
               end do
            end do
          else
            do isub=1,avg_blk%nsub
