            ...
            if (valid) then
             ...
             & lct_blk%D(:,:,ic1a,il0),lct_blk%coef(:,ic1a,il0),lct_blk%fit(:,:,ic1a,il0))
               ...
               do jl0r=1,bpar%nl0r(ib)
                  ...
                  do jc3=1,nam%nc3
                     if (samp%c1c3l0_log(ic1,jc3,jl0)) then
                        if (mpl%msv%isnotr(lct_blk%fit(jc3,jl0r,ic1a,il0)).and.lct_blk%raw(jc3,jl0r,ic1a,il0)>nam%lct_qc_th) then
...
do iscales=1,lct_blk%nscales
   ...
   do il0=1,geom%nl0
      ...
      call mpl%f_comm%allreduce(sum(fld(:,il0,2*4+4),mpl%msv%isnotr(fld(:,il0,2*4+4))),Lavg_tot,fckit_mpi_sum())
      call mpl%f_comm%allreduce(real(count(mpl%msv%isnotr(fld(:,il0,2*4+4))),kind_real),norm_tot,fckit_mpi_sum())
