...
! Setup sampling
call lct%samp%setup_sampling(mpl,rng,nam,geom,bpar,ens)
...
do ib=1,bpar%nb
   if (bpar%diag_block(ib)) then
      write(filename,'(a,i4.4,a,i4.4,a,a)') trim(nam%prefix)//'_lct_cor_',mpl%nproc,'-',mpl%myproc,'_',trim(bpar%blockname(ib))
