...
! Initialization
curve%npack = (nam%nc*geom%nl0+3)*geom%nl0
call msr(curve%raw)
...
do jl0=1,geom%nl0
   do il0=1,bpar%nl0(ib)
      do ic=1,bpar%icmax(ib)
         if (isnotmsr(curve%raw(ic,il0,jl0)).and.isnotmsr(curve%raw_coef_ens(il0)).and.isnotmsr(curve%raw_coef_ens(jl0))) &
       & curve%raw(ic,il0,jl0) = curve%raw(ic,il0,jl0)/sqrt(curve%raw_coef_ens(il0)*curve%raw_coef_ens(jl0))
      end do
   end do
end do
...
! Pack
offset = 0
buf(offset+1:offset+nam%nc*geom%nl0**2) = pack(curve%fit,.true.)
offset = offset+nam%nc*geom%nl0**2
...
! Allocation
allocate(mask_unpack(nam%nc,geom%nl0,geom%nl0))
mask_unpack = .true.

! Unpack
offset = 0
curve%fit = unpack(buf(offset+1:offset+nam%nc*geom%nl0**2),mask_unpack,curve%fit)
offset = offset+nam%nc*geom%nl0**2
