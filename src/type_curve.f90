...
private
public :: curvetype
public :: curve_alloc,curve_dealloc,curve_normalization,curve_write
...
! Passed variables
type(hdatatype),intent(in) :: hdata         !< Sampling data
type(curvetype),intent(inout) :: curve(hdata%nam%nvp) !< Curve
...
do iv=1,nam%nvp
   ! Get diagonal values
   do il0=1,geom%nl0
      if (isnotmsr(curve(iv)%raw(1,il0,il0))) then
          curve(iv)%raw_coef_ens(il0) = curve(iv)%raw(1,il0,il0)
      else
          call msgerror('missing raw value at zero separation')
      end if
      if (trim(nam%fit_type)/='none') then
         if (isnotmsr(curve(iv)%fit(1,il0,il0))) curve(iv)%fit_coef_ens(il0) = curve(iv)%fit(1,il0,il0)
      else
   ...
   if (nam%norm_loc) then
      do jl0=1,geom%nl0
         do il0=1,geom%nl0
            do ic=1,nam%nc
               if (isnotmsr(curve(iv)%raw_coef_ens(il0)).and.isnotmsr(curve(iv)%raw_coef_ens(jl0)) &
             & .and.isnotmsr(curve(iv)%raw(ic,il0,jl0))) &
             & curve(iv)%raw(ic,il0,jl0) = curve(iv)%raw(ic,il0,jl0) &
             & /sqrt(curve(iv)%raw_coef_ens(il0)*curve(iv)%raw_coef_ens(jl0))
               if (trim(nam%fit_type)/='none') then
                  if (isnotmsr(curve(iv)%fit_coef_ens(il0)).and.isnotmsr(curve(iv)%fit_coef_ens(jl0)) &
                & .and.isnotmsr(curve(iv)%fit(ic,il0,jl0))) &
                & curve(iv)%fit(ic,il0,jl0) = curve(iv)%fit(ic,il0,jl0) &
                & /sqrt(curve(iv)%fit_coef_ens(il0)*curve(iv)%fit_coef_ens(ic))
               end if
            end do
         end do
      end do
...
!> Purpose: write a set of curve
!----------------------------------------------------------------------
subroutine curve_write(hdata,ncid,curve,lcom)
...
type(curvetype),intent(inout) :: curve(:)  !< Curves
logical,intent(in) :: lcom                  !< Key for common diagnostics
...
! Define the number of variables to write
if (lcom) then
   nveff = nam%nvp
else
   nveff = nam%nv
...
do iv=1,nveff
   ! Compute spectra
   if (nam%spectrum) call curve_spectra(hdata,curve(iv))
   ...
   ! Raw curve
   call ncerr(subr,nf90_def_var(ncid,trim(curve(iv)%cname)//'_raw',ncfloat,(/nc_id,nl0_1_id,nl0_2_id/),raw_id))
   call ncerr(subr,nf90_put_att(ncid,raw_id,'_FillValue',msvalr))
   ! Raw curve ensemble coefficient
   call ncerr(subr,nf90_def_var(ncid,trim(curve(iv)%cname)//'_raw_coef_ens',ncfloat,(/nl0_1_id/),raw_coef_ens_id))
   call ncerr(subr,nf90_put_att(ncid,raw_coef_ens_id,'_FillValue',msvalr))
   if (nam%spectrum) then
      ! Raw curve eigenspectrum
      call ncerr(subr,nf90_def_var(ncid,trim(curve(iv)%cname)//'_raw_spec',ncfloat,(/nc_id,nl0_1_id,nl0_2_id/),raw_spec_id))
      call ncerr(subr,nf90_put_att(ncid,raw_spec_id,'_FillValue',msvalr))
   end if
   if (isnotmsr(curve(iv)%raw_coef_sta)) then
      ! Raw curve static coefficient
      call ncerr(subr,nf90_def_var(ncid,trim(curve(iv)%cname)//'_raw_coef_sta',ncfloat,(/one_id/),raw_coef_sta_id))
      call ncerr(subr,nf90_put_att(ncid,raw_coef_sta_id,'_FillValue',msvalr))
   end if
   if (trim(nam%fit_type)/='none') then
      ! Fitted curve
      call ncerr(subr,nf90_def_var(ncid,trim(curve(iv)%cname)//'_fit',ncfloat,(/nc_id,nl0_1_id,nl0_2_id/),fit_id))
      call ncerr(subr,nf90_put_att(ncid,fit_id,'_FillValue',msvalr))
      ! Fitted curve support radius
      call ncerr(subr,nf90_def_var(ncid,trim(curve(iv)%cname)//'_fit_rh',ncfloat,(/nl0_1_id/),fit_rh_id))
      call ncerr(subr,nf90_put_att(ncid,fit_rh_id,'_FillValue',msvalr))
      ! Fitted curve support radius
      call ncerr(subr,nf90_def_var(ncid,trim(curve(iv)%cname)//'_fit_rv',ncfloat,(/nl0_1_id/),fit_rv_id))
      call ncerr(subr,nf90_put_att(ncid,fit_rv_id,'_FillValue',msvalr))
      ! Fitted curve ensemble coefficient
      call ncerr(subr,nf90_def_var(ncid,trim(curve(iv)%cname)//'_fit_coef_ens',ncfloat,(/nl0_1_id/),fit_coef_ens_id))
      call ncerr(subr,nf90_put_att(ncid,fit_coef_ens_id,'_FillValue',msvalr))
      if (nam%spectrum) then
         ! Fitted curve eigenspectrum
         call ncerr(subr,nf90_def_var(ncid,trim(curve(iv)%cname)//'_fit_spec',ncfloat,(/nc_id,nl0_1_id,nl0_2_id/),fit_spec_id))
   ...
   ! Raw curve
   call ncerr(subr,nf90_put_var(ncid,raw_id,curve(iv)%raw))
   ! Raw curve ensemble coefficient
   call ncerr(subr,nf90_put_var(ncid,raw_coef_ens_id,curve(iv)%raw_coef_ens))
   if (nam%spectrum) then
      ! Raw curve eigenspectrum
      call ncerr(subr,nf90_put_var(ncid,raw_spec_id,curve(iv)%raw_spec))
   end if
   ! Raw curve static coefficient
   if (isnotmsr(curve(iv)%raw_coef_sta)) call ncerr(subr,nf90_put_var(ncid,raw_coef_sta_id,curve(iv)%raw_coef_sta))
   if (trim(nam%fit_type)/='none') then
      ! Fitted curve
      call ncerr(subr,nf90_put_var(ncid,fit_id,curve(iv)%fit))
      ! Fitted curve ensemble coefficient
      call ncerr(subr,nf90_put_var(ncid,fit_coef_ens_id,curve(iv)%fit_coef_ens))
      ! Fitted curve support radius
      call ncerr(subr,nf90_put_var(ncid,fit_rh_id,curve(iv)%fit_rh))
      ! Fitted curve support radius
      call ncerr(subr,nf90_put_var(ncid,fit_rv_id,curve(iv)%fit_rv))
      if (nam%spectrum) then
         ! Fitted curve eigenspectrum
         call ncerr(subr,nf90_put_var(ncid,fit_spec_id,curve(iv)%fit_spec))
      end if
   end if
end do
