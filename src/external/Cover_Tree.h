...
class CoverTree
{
    ...
    public:
        ...
        // To set and get _dist
        void setDist(double dist) { _dist = dist; }
 ...
 public:
     ...
     */
    const std::vector<Point> kNearestNeighbors(const Point& p, const unsigned int& k) const;
...
CoverTree<Point>::kNearestNodes(const Point& p, const unsigned int& k) const
{
    ...
    double d = p.distance(_root->getPoint());
    _root->setDist(d);
...
template<class Point>
const std::vector<Point> CoverTree<Point>::kNearestNeighbors(const Point& p,
...
void CoverTree<Point>::print() const
{
    ...
    for(int i=0;i<d;i++) {
        ...
        typename std::vector<CoverTreeNode*>::const_iterator it;
        for(it=Q.begin();it!=Q.end();++it) {
            std::vector<CoverTreeNode*> children = (*it)->getChildren(_maxLevel-i);
...
template<class Point>
const std::vector<Point>& CoverTree<Point>::CoverTreeNode::getPoints()
{
    double dist = this->getDist();
    for(unsigned int i=0;i<_points.size();i++) {
        _points[i].setDist(dist);
    }
    return _points;
}
