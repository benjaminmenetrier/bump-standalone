...
type hdatatype
   ...
   ! Sampling
   integer,allocatable :: ic1_to_ic0(:)               !< Horizontal sampling index
   logical,allocatable :: ic1il0_log(:,:)             !< Log for the horizontal sampling index
   integer,allocatable :: ic1icil0iv_to_ic0(:,:,:,:)         !< Second horizontal sampling index
   logical,allocatable :: ic1icil0iv_log(:,:,:,:)         !< Log for the second horizontal sampling index
   ...
   type(ctreetype),allocatable :: ctree_cell(:) !< Cells cover tree
   integer,allocatable :: nn_nc1_index(:,:,:)  !< Nearest diagnostic neighbors from diagnostic points
   real(kind_real),allocatable :: nn_nc1_dist(:,:,:)      !< Nearest diagnostic neighbors distance from diagnostic points
   ...
   ! Interpolation from diagnostics to grid cells
...
allocate(hdata%ic1icil0iv_to_ic0(nam%nc1,nam%nc,geom%nl0,nam%nv))
allocate(hdata%ic1icil0iv_log(nam%nc1,nam%nc,geom%nl0,nam%nv))
allocate(hdata%swgt(nam%nc1,nam%nc,geom%nl0,geom%nl0,nam%nv))
allocate(hdata%bwgtsq(nam%nc,geom%nl0,geom%nl0,nam%nv))
if (nam%local_diag.or.nam%displ_diag) then
   allocate(hdata%ic2_to_ic1(hdata%nc2))
   allocate(hdata%ic2_to_ic0(hdata%nc2))
end if
if (nam%local_diag.or.nam%displ_diag) then
   allocate(hdata%nn_nc1_index(nam%nc1,hdata%nc2,geom%nl0i))
   allocate(hdata%nn_nc1_dist(nam%nc1,hdata%nc2,geom%nl0i))
...
call msi(hdata%ic1icil0iv_to_ic0)
hdata%ic1icil0iv_log = .false.
call msr(hdata%swgt)
...
if (nam%local_diag.or.nam%displ_diag) then
   call msi(hdata%nn_nc1_index)
   call msr(hdata%nn_nc1_dist)
   if (trim(nam%flt_type)/='none') then
...
deallocate(hdata%ic1icil0iv_to_ic0)
deallocate(hdata%ic1icil0iv_log)
deallocate(hdata%swgt)
deallocate(hdata%bwgtsq)
if (nam%local_diag.or.nam%displ_diag) then
   deallocate(hdata%ic2_to_ic1)
   deallocate(hdata%ic2_to_ic0)
   deallocate(hdata%nn_nc1_index)
   deallocate(hdata%nn_nc1_dist)
   if (trim(nam%flt_type)/='none') then
      deallocate(hdata%nn_nc2_index)
      deallocate(hdata%nn_nc2_dist)
   end if
end if
if (nam%displ_diag.and..not.nam%displ_explicit) then
   do il0=1,geom%nl0
      call delete_ctree(hdata%ctree_cell(il0))
   end do
   deallocate(hdata%ctree_cell)
end if
if (nam%local_diag.or.nam%displ_diag) then
   do il0=1,geom%nl0
...
! Local variables
integer :: iv,il0,il0i,ic1,ic
integer :: nl0_1_test,nl0_2_test,nc_test,nc1_test,nc2_test
integer :: info,ncid,nl0_1_id,nl0_2_id,nc_id,nc1_id,nc2_id
integer :: ic1_to_ic0_id,ic1il0_log_id,ic1icil0iv_to_ic0_id,ic1icil0iv_log_id,swgt_id
integer :: ic2_to_ic1_id,ic2_to_ic0_id,nn_nc1_index_id,nn_nc1_dist_id,nn_nc2_index_id,nn_nc2_dist_id
integer :: ic1il0_logint(hdata%nam%nc1,hdata%geom%nl0),ic1icil0iv_logint(hdata%nam%nc1,hdata%nam%nc,hdata%geom%nl0)
...
call ncerr(subr,nf90_inq_varid(ncid,'ic1icil0iv_to_ic0',ic1icil0iv_to_ic0_id))
call ncerr(subr,nf90_inq_varid(ncid,'ic1icil0iv_log',ic1icil0iv_log_id))
...
call ncerr(subr,nf90_get_var(ncid,ic1icil0iv_to_ic0_id,hdata%ic1icil0iv_to_ic0(:,:,:,1)))
call ncerr(subr,nf90_get_var(ncid,ic1icil0iv_log_id,ic1icil0iv_logint))
do il0=1,geom%nl0
   do ic=1,nam%nc
      do ic1=1,nam%nc1
         if (ic1icil0iv_logint(ic1,ic,il0)==0) then
            hdata%ic1icil0iv_log(ic1,ic,il0,1) = .false.
         else if (ic1icil0iv_logint(ic1,ic,il0)==1) then
            hdata%ic1icil0iv_log(ic1,ic,il0,1) = .true.
         end if
      end do
   end do
end do
call ncerr(subr,nf90_get_var(ncid,swgt_id,hdata%swgt(:,:,:,:,1)))
if ((hdata_read==0).and.(nam%local_diag.or.nam%displ_diag)) then
...
do iv=2,nam%nv
   hdata%ic1icil0iv_to_ic0(:,:,:,iv) = hdata%ic1icil0iv_to_ic0(:,:,:,1)
   hdata%ic1icil0iv_log(:,:,:,iv) = hdata%ic1icil0iv_log(:,:,:,1)
   hdata%swgt(:,:,:,:,iv) = hdata%swgt(:,:,:,:,1)
end do
...
if ((hdata_read==0).and.(nam%local_diag.or.nam%displ_diag)) then
   do il0i=1,geom%nl0i
      ...
      call ncerr(subr,nf90_inq_varid(ncid,'nn_nc1_index',nn_nc1_index_id))
      call ncerr(subr,nf90_inq_varid(ncid,'nn_nc1_dist',nn_nc1_dist_id))
      call ncerr(subr,nf90_get_var(ncid,nn_nc1_index_id,hdata%nn_nc1_index(:,:,il0i)))
      call ncerr(subr,nf90_get_var(ncid,nn_nc1_dist_id,hdata%nn_nc1_dist(:,:,il0i)))
      if (trim(nam%flt_type)/='none') then
...
! Local variables
integer :: il0,il0i,ic1,ic
integer :: ncid,nl0_1_id,nl0_2_id,nc1_id,nc2_1_id,nc2_2_id,nc_id,ndata_id
integer :: lat_id,lon_id,smax_id,ic1_to_ic0_id,ic1il0_log_id,ic1icil0iv_to_ic0_id,ic1icil0iv_log_id,swgt_id
integer :: ic2_to_ic1_id,ic2_to_ic0_id,nn_nc1_index_id,nn_nc1_dist_id,nn_nc2_index_id,nn_nc2_dist_id
integer :: ic1il0_logint(hdata%nam%nc1,hdata%geom%nl0),ic1icil0iv_logint(hdata%nam%nc1,hdata%nam%nc,hdata%geom%nl0)
...
call ncerr(subr,nf90_def_var(ncid,'ic1icil0iv_to_ic0',nf90_int,(/nc1_id,nc_id,nl0_1_id/),ic1icil0iv_to_ic0_id))
call ncerr(subr,nf90_put_att(ncid,ic1icil0iv_to_ic0_id,'_FillValue',msvali))
call ncerr(subr,nf90_def_var(ncid,'ic1icil0iv_log',nf90_int,(/nc1_id,nc_id,nl0_1_id/),ic1icil0iv_log_id))
call ncerr(subr,nf90_put_att(ncid,ic1icil0iv_log_id,'_FillValue',msvali))
...
do il0=1,geom%nl0
   do ic=1,nam%nc
      do ic1=1,nam%nc1
         if (hdata%ic1icil0iv_log(ic1,ic,il0,1)) then
            lon(ic1,ic,il0) = hdata%geom%lon(hdata%ic1icil0iv_to_ic0(ic1,ic,il0,1))*rad2deg
            lat(ic1,ic,il0) = hdata%geom%lat(hdata%ic1icil0iv_to_ic0(ic1,ic,il0,1))*rad2deg
         end if
      end do
   end do
end do
...
call ncerr(subr,nf90_put_var(ncid,smax_id,float(count(hdata%ic1icil0iv_log(:,:,:,1),dim=1))))
...
call ncerr(subr,nf90_put_var(ncid,ic1icil0iv_to_ic0_id,hdata%ic1icil0iv_to_ic0(:,:,:,1)))
do il0=1,geom%nl0
   do ic=1,nam%nc
      do ic1=1,nam%nc1
         if (hdata%ic1icil0iv_log(ic1,ic,il0,1)) then
            ic1icil0iv_logint(ic1,ic,il0) = 1
         else
            ic1icil0iv_logint(ic1,ic,il0) = 0
         end if
      end do
   end do
end do
call ncerr(subr,nf90_put_var(ncid,ic1icil0iv_log_id,ic1icil0iv_logint))
call ncerr(subr,nf90_put_var(ncid,swgt_id,hdata%swgt(:,:,:,:,1)))
if (nam%local_diag.or.nam%displ_diag) then
...
if (nam%local_diag.or.nam%displ_diag) then
   do il0i=1,geom%nl0i
    ...
    & or(nf90_clobber,nf90_64bit_offset),ncid))
      ...
      call ncerr(subr,nf90_def_var(ncid,'nn_nc1_index',nf90_int,(/nc1_id,nc2_1_id/),nn_nc1_index_id))
      call ncerr(subr,nf90_put_att(ncid,nn_nc1_index_id,'_FillValue',msvali))
      call ncerr(subr,nf90_def_var(ncid,'nn_nc1_dist',ncfloat,(/nc1_id,nc2_1_id/),nn_nc1_dist_id))
      call ncerr(subr,nf90_put_att(ncid,nn_nc1_dist_id,'_FillValue',msvalr))
      if (trim(nam%flt_type)/='none') then
      ...
      call ncerr(subr,nf90_enddef(ncid))
      call ncerr(subr,nf90_put_var(ncid,nn_nc1_index_id,hdata%nn_nc1_index(:,:,il0i)))
      call ncerr(subr,nf90_put_var(ncid,nn_nc1_dist_id,hdata%nn_nc1_dist(:,:,il0i)))
      if (trim(nam%flt_type)/='none') then
