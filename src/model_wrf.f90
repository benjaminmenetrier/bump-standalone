...
do iv=1,nam%nv
   ...
   do il0=1,nam%nl
      ...
      case ('U')
         ...
         fld(:,il0,iv) = pack(real(0.5*(fld_loc(1:geom%nlon,:)+fld_loc(2:geom%nlon+1,:)),kind_real),mask=.true.)
      case ('V')
         call ncerr(subr,nf90_get_var(ncid,fld_id,fld_loc,(/1,1,nam%levs(il0),nam%timeslot(its)/),(/geom%nlon,geom%nlat+1,1,1/)))
         fld(:,il0,iv) = pack(real(0.5*(fld_loc(:,1:geom%nlat)+fld_loc(:,2:geom%nlat+1)),kind_real),mask=.true.)
      case default
