...
if (nam%split_io.and.(mpl%nproc>1)) then
   ...
   write(filename_proc,'(a,a,i4.4,a,i4.4)') trim(filename),'_split_',mpl%nproc,'-',mpl%myproc
   ...
   if (info==nf90_noerr) then
      ...
      ! Open file
      call mpl%ncerr(subr,nf90_open(trim(nam%datadir)//'/'//trim(filename_proc)//'.nc',nf90_write,ncid))
   ...
   nc0a_id = mpl%ncdimcheck(subr,ncid,'nc0a',geom%nc0a,.true.)
