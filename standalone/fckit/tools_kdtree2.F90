...
!----------------------------------------------------------------------
module tools_kdtree2
  use fckit_geometry_module, only: sphere_xyz2lonlat,sphere_distance
...
! Purpose: spherical distance between iv[1:n] and qv[1:n]
    ...
    call sphere_xyz2lonlat(iv(1),iv(2),iv(3),ilon,ilat)
    call sphere_xyz2lonlat(qv(1),qv(2),qv(3),qlon,qlat)

    ! Distance on sphere
    res = sphere_distance(ilon,ilat,qlon,qlat)
